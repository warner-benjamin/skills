...
description: Execute a human-approved managed workflow plan with cost-aware Luna-first multi_agent_v1 workers, escalation to Terra or Sol based on complexity and risk, Sol integration, persistent goal state, and implementation verification. Use when the user explicitly invokes $managed-implement or asks to execute an existing ready managed plan. Do not use until the user has reviewed the plan and explicitly authorized implementation.
---

# Managed implement

Execute the ready plan while keeping Sol responsible for scope, integration, and completion.
...
- `Plan: ready`
- `Plan review: not-required` or `passed`, or `unavailable` when the checklist records the Sol fallback and independence was not required
...
Allow Sol to implement locally when no runner is available, the work is too small to justify transferring context, the work is tightly coupled to active integration, or one focused worker retry failed. Record the reason briefly when a checklist exists.
...
3. Let the worker implement and run targeted checks.
...
7. Mark the item `complete` only when its acceptance condition and targeted checks pass. Record concise evidence.
...
After incomplete worker output or work outside the assigned scope, retry once with a tighter prompt. Then reslice, let Sol take over, or return to planning.
...
5. Follow the shared commit policy. Leave the normal final commit until after quality and final verification.
