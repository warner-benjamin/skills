...
- Consult the nearby pages, components, state patterns, shared primitives, and product copy relevant to the change.
...
- missing loading, empty, error, disabled, success, recovery, permission, long-content, or many-row behavior required by the product contract;
...
Exercise representative narrow-screen, larger-text, long-label, missing-value, overflow, many-row, asynchronous, and recovery states proportional to the change. A desktop layout that merely shrinks without deciding mobile priority is incomplete.
