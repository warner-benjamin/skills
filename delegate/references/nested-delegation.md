...
Carry the skill's major-decision guidance into each non-Luna child packet: ask the direct parent through `collaboration.send_message` before committing to major changes, with evidence, options, and a recommendation. Resolve routine details locally. Escalate beyond the direct parent only when the decision exceeds that parent's scope or authority; a Luna child returns blocking decisions in its final report.
