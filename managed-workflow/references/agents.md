...
A request for `$managed-workflow`, `$managed-plan`, or a reviewed managed workflow plan authorizes exactly one independent plan reviewer plus same-thread re-review for the planning phase. It does not authorize implementation workers, slice reviewers, final quality reviewers, parallel agents, or larger workgroups until the relevant workflow phase and user gate allow them.
