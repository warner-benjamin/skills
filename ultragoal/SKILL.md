...
Use the model and effort selected by the user, governing source, or activation packet when the choice remains available and appropriate. Otherwise inherit the parent model and effort. Resolve their exact values before dispatch; when the runtime does not expose them, pass an explicit compatible model and effort so the dispatch can be reported accurately. Use this table only when an override materially helps. Increase Luna effort for reasoning depth, use Terra when the work needs broader knowledge, and use Sol for consequential decisions or reviews. Make the work packet complete before assigning a stronger model. Do not assign several models to compete on the same task.
...
Sol and Terra workers may delegate bounded child slices of their own assignment. Before spawning a child, they must load and apply this Ultragoal skill as that child's parent, including its ownership, task-packet, wait, verification, reuse, and dispatch-notice rules. Nested delegation must remain within the active goal and the worker's own scope, authority, write boundaries, invariants, and available concurrency; it must not expand the task or create overlapping work.
